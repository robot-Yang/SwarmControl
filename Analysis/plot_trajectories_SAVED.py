...
# ===== CONFIGURATION =====
# Set the reference trajectory filename here (must be in the Analysis folder)
# Leave as None to disable comparison plotting
REFERENCE_TRAJECTORY = "reference.json"  # Example: "reference_run.json"
# =========================
...
def compute_swarm_centroid_trajectory(data, height_deadzone=1.0, filter_trial=True):
    ...
    Args:
        ...
        filter_trial: If True and trial data exists, only use data within trial window
    """
    # Determine time window to analyze
    start_time = 0
    end_time = float('inf')
    
    if filter_trial and data.get('trials'):
        # Use the first 'Run' trial window
        ...
        if run_trial:
            ...
            print(f"Filtering trajectory data to trial window: {start_time:.2f}s to {end_time:.2f}s (duration: {end_time-start_time:.2f}s)")
    
    # Collect all unique time points within the time window
    time_points = set()
...
def plot_2d_trajectories(data, show_connectivity=True, height_deadzone=0.0, filter_trial=True):
    ...
    Args:
        ...
        filter_trial: If True, only plot data within trial window
    """
    fig, ax = plt.subplots(figsize=(12, 10))
   
    # Compute swarm centroid
    times, cx, cy, cz = compute_swarm_centroid_trajectory(data, height_deadzone=height_deadzone, filter_trial=filter_trial)
...
def plot_3d_trajectories(data, height_deadzone=0.2, filter_trial=True):
...
def plot_comparison_3d(latest_data, reference_data, height_deadzone=0.2, filter_trial=True):
    """Plot both latest and reference trajectories on the same 3D plot.
    
    Args:
        latest_data: Most recent trajectory data
        reference_data: Reference trajectory data to compare against
        height_deadzone: Clamp height variations smaller than this value (meters)
        filter_trial: If True, only plot data within trial windows
    """
    fig = plt.figure(figsize=(16, 10))
    ax = fig.add_subplot(111, projection='3d')
    
    # Compute centroids for both trajectories
    times_latest, cx_latest, cy_latest, cz_latest = compute_swarm_centroid_trajectory(latest_data, height_deadzone=height_deadzone, filter_trial=filter_trial)
    times_ref, cx_ref, cy_ref, cz_ref = compute_swarm_centroid_trajectory(reference_data, height_deadzone=height_deadzone, filter_trial=filter_trial)
    
    # Plot latest trajectory in blue
    if len(cx_latest) > 0:
        ax.plot(cx_latest, cz_latest, cy_latest, 'b-', linewidth=2.5, alpha=0.8, label=f'Latest: {latest_data["scene"]}')
        ax.scatter(cx_latest[0], cz_latest[0], cy_latest[0], c='blue', marker='o', s=150, label='Latest Start', zorder=5)
        ax.scatter(cx_latest[-1], cz_latest[-1], cy_latest[-1], c='blue', marker='s', s=150, label='Latest End', zorder=5)
    
    # Plot reference trajectory in red
    if len(cx_ref) > 0:
        ax.plot(cx_ref, cz_ref, cy_ref, 'r-', linewidth=2.5, alpha=0.8, label=f'Reference: {reference_data["scene"]}')
        ax.scatter(cx_ref[0], cz_ref[0], cy_ref[0], c='red', marker='o', s=150, label='Ref Start', zorder=5)
        ax.scatter(cx_ref[-1], cz_ref[-1], cy_ref[-1], c='red', marker='s', s=150, label='Ref End', zorder=5)
    
    ax.set_xlabel('X Position (m) - Left/Right', fontsize=12)
    ax.set_ylabel('Z Position (m) - Forward/Back', fontsize=12)
    ax.set_zlabel('Y Position (m) - Height', fontsize=12)
    ax.set_title(f"Trajectory Comparison: Latest vs Reference", fontsize=14, fontweight='bold')
    
    # Set equal aspect ratio based on combined data
    all_x = np.concatenate([cx_latest, cx_ref]) if len(cx_latest) > 0 and len(cx_ref) > 0 else (cx_latest if len(cx_latest) > 0 else cx_ref)
    all_z = np.concatenate([cz_latest, cz_ref]) if len(cz_latest) > 0 and len(cz_ref) > 0 else (cz_latest if len(cz_latest) > 0 else cz_ref)
    all_y = np.concatenate([cy_latest, cy_ref]) if len(cy_latest) > 0 and len(cy_ref) > 0 else (cy_latest if len(cy_latest) > 0 else cy_ref)
...
def main():
    ...
    # Load reference trajectory if configured
    if REFERENCE_TRAJECTORY is not None:
        reference_path = Path(__file__).parent / REFERENCE_TRAJECTORY
        if reference_path.exists():
            print(f"\nLoading reference trajectory: {REFERENCE_TRAJECTORY}")
            reference_data = load_trajectory_data(reference_path)
            print("  - Creating comparison plot...")
            fig_comparison = plot_comparison_3d(data, reference_data)
        else:
            print(f"\nWarning: Reference trajectory not found: {reference_path}")
